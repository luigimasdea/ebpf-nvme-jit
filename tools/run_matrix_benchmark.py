...
def run_benchmark(chunk_size):
    ...
    for line in output.split('\n'):
        if "Total Time" in line:
            # Example: "  Total Time              |        1318.52 ms |        1273.45 ms |        1064.21 ms"
            ...
            # Example: "  Effective Throughput    |       106.94 MB/s|       110.72 MB/s|       132.50 MB/s"
            ...
            # Example: "  Host Data Reduction     |             0.0% |           96.7% |           96.7%"
            match = re.findall(r'(\d+\.\d+)%', line)
            if len(match) >= 3:
                metrics["data_reduction"] = float(match[1])  # taking the CSD one
...
def main():
    ...
    print("Starting automated matrix benchmarking...")
    ...
    print("\n\n==============================================================================================")
    print("                               BENCHMARK RESULTS (Mean ± StdDev)                              ")
    print("==============================================================================================")
    print(f"{'Chunk':<8} | {'Mode':<15} | {'Total Time (ms)':<25} | {'Throughput (MB/s)':<25}")
    print("-" * 85)
    ...
    for chunk in chunk_sizes:
        modes = [
            ("Host Traditional", "host_time", "host_thru"),
        ...
        for idx, (mode_name, time_key, thru_key) in enumerate(modes):
            ...
            print(f"{chunk_str:<8} | {mode_name:<15} | {time_mean:>8.2f} ± {time_std:<8.2f} ms      | {thru_mean:>8.2f} ± {thru_std:<8.2f} MB/s")
    ...
    # Write to CSV
    csv_file = "benchmark_matrix_results.csv"
