...
void emit_alu(struct ebpf_inst inst, uint8_t rd, uint8_t rs, uint8_t f3,
              uint8_t f7) {
 ...
 * ========================================================================= */
static void generate_insn(struct ebpf_inst inst) {
  uint8_t op = inst.opcode;
  ...
  case BPF_ALU64:
    switch (BPF_OP(op)) {
    case BPF_ADD:
      emit_alu(inst, rd, rs, RV_F3_ADD, RV_F7_ADD);
      break;
    case BPF_SUB:
      emit_alu(inst, rd, rs, RV_F3_ADD, RV_F7_SUB);
      break;
    case BPF_AND:
      emit_alu(inst, rd, rs, RV_F3_AND, RV_F7_ADD);
      break;
    case BPF_OR:
      emit_alu(inst, rd, rs, RV_F3_OR, RV_F7_ADD);
      break;
    case BPF_XOR:
      emit_alu(inst, rd, rs, RV_F3_XOR, RV_F7_ADD);
      break;
    case BPF_MUL:
      emit_alu(inst, rd, rs, RV_F3_MUL, RV_F7_MUL);
      break;
    case BPF_DIV:
      emit_alu(inst, rd, rs, RV_F3_DIV, RV_F7_MUL);
      break;
    case BPF_MOD:
      emit_alu(inst, rd, rs, RV_F3_REM, RV_F7_MUL);
      break;
    case BPF_LSH:
      emit_alu(inst, rd, rs, RV_F3_SLL, RV_F7_ADD);
      break;
    case BPF_RSH:
      emit_alu(inst, rd, rs, RV_F3_SRL, RV_F7_ADD);
      break;
    case BPF_ARSH:
      emit_alu(inst, rd, rs, RV_F3_SRL, RV_F7_SRA);
      break;

    case BPF_MOV:
      if (BPF_SRC(op) == BPF_K) {
        if (inst.imm >= -2048 && inst.imm <= 2047) {
          emit_rv32(RV_MAKE_I(is_alu64 ? RV_OP_IMM : RV_OP_IMM_32, rd,
                              RV_F3_ADD, RV_REG_ZERO, inst.imm));
        } else {
          emit_load_imm(RV_REG_T0, inst.imm);
          emit_rv32(RV_MAKE_I(is_alu64 ? RV_OP_IMM : RV_OP_IMM_32, rd,
                              RV_F3_ADD, RV_REG_T0, 0));
        }
      } else {
        emit_rv32(RV_MAKE_I(is_alu64 ? RV_OP_IMM : RV_OP_IMM_32, rd, RV_F3_ADD,
                            rs, 0));
      }
      break;

    case BPF_NEG:
      emit_rv32(RV_MAKE_R(is_alu64 ? RV_OP_ALU : RV_OP_ALU_32, rd, RV_F3_ADD,
                          RV_REG_ZERO, rd, RV_F7_SUB));
      break;
    }
    break;
